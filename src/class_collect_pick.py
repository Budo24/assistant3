...
class PickAndCollect:
    ...
    def collect_order_with_id(self, order_id):
        if self.pick_order_ability(order_id) == 'not collected':
            ...
            collect_info = [object_name, object_amount, corridor_number, rack_number]
            collect_info = dict(
        ...
        try:
            while True:
                for to_pick_order in json_order:
                    status = self.order_status(corridor_number, to_pick_order['rack_number'])
                    if status[0] == ['not picked', 'not collected']:
