"""Tests for the GUI."""
from assistant3.gui.gui_class import GUI
...
def test_speech_update():
    ...
    assert GUI().__init__().lbl.getc("text") == "This is a new Text"
...
def test_animate():
    ...
    update(rec=b'0000')
    assert not GUI().__init__.animate

    update(rec=b'0001')
    assert GUI().__init__.animate
